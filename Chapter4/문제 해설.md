...
```
btn1의 버튼이 클릭되어 콜백함수(화살표함수)로 등록된 함수가 실행되어 실행 컨텍스트를 만들고 실행시킬때, 함수의 O.E에는 선언될 당시의 L.E를 참조하기떄문에, 마찬가지로 콜백함수의 this또한  버튼을 클릭할 당시가아닌(함수가 실행될당시가아닌) 선언될당시의 스코프 체인상 가장 가까운 실행컨텍스트의 this를 참조하기떄문에 , 버튼을 클릭하여 콜백함수를 실행시킬때 this값으로 f1을 가리키게 됩니다.
```
