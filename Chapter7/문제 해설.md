...
g.length를 지웠기 때문에 g에서는 length가 없다. 따라서 g.push를 할 때 자동적으로 프로토타입 체이닝이 되면서 g.__proto__.length를 참조하게 된다.
이때 Grade.prototype이 배열 [1, 2, 3, 4, 5]이기 때문에 이것의 length는 5가 된다. 따라서 배열의 5번째 자리에 70이 들어가고, length는 6이 되는 것이다. 
```
